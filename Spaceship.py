...
class Ship:
    ...
    def thrusters(self, on):
        if on:
            ...
            self._thrusters = True
        else:
            self.image_center[0] -= self.image_size[0]
            self._thrusters = False
            
    def change_dir(self, key_down, clockwise):
        self._rotate = key_down
        if not key_down:
            return
...
def draw(canvas):
    ...
    # draw the UI
    canvas.draw_text("lives", [50, 100], 25, "White")
    canvas.draw_text(str(lives), [130, 100], 25, "White")
    canvas.draw_text("score", [680, 100], 25, "White")
    canvas.draw_text(str(score), [750, 100], 25, "White")
            
# timer handler that spawns a rock    
def rock_spawner():
    global a_rock
    rock_vel = [0, 0]
    rock_vel[0] = (random.randrange(-10, 10) / 10.0)
    rock_vel[1] = (random.randrange(-10, 10) / 10.0)
    rock_ang_vel = (random.randrange(-10, 10) / 100.0)
