...
class Sprite:
    ...
    def collide(self, other_object):
        dist = math.sqrt( ((self.pos[0] - other_object.get_pos()[0]) ** 2 +
                           (self.pos[1] - other_object.get_pos()[1]) ** 2) )
        if dist <= self.radius + other_object.get_radius():
            return True
        else:
            return False;
...
def rock_spawner():
    ...
    while score // base > 0:
        base *= 10
    factor = dit[base]
    rock_vel = [random.random() * factor - .3, random.random() * factor - .3]
    rock_avel = random.random() * .2 - .1
    a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
    # if a rock spawn on top of the ship ignore it
...
def group_collide(sprite_group, other_obj):
    ...
    for i in sprite_group:
        if i.collide(other_obj):
            remove.append(i)
            tmp = Sprite(other_obj.get_pos(), [0, 0], 0, 24, explosion_image, explosion_info, explosion_sound)
            explosion_group.add(tmp)
